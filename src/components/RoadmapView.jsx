...
function statusLabel(deadline, done) {
  ...
  if (diff < 7 * DAY_MS) return `${Math.ceil(diff / DAY_MS)}d`;
...
export default function RoadmapView({ regions, onSelectBed }) {
  ...
  const rangeMs = rangeEnd.getTime() - today.getTime();
    ...
    <section className="roadmap">
      ...
      <div className="roadmap-timeline">
        ...
        <div className="roadmap-rows">
          {fruits.length === 0 && (
            <div className="roadmap-empty">No fruits with deadlines yet. Add fruits to your plants to see them here.</div>
          )}
          {fruits.map((f) => {
            const t = threadById[f.thread];
            const dlMs = f.deadline;
            const offsetDays = Math.max(0, (dlMs - today.getTime()) / DAY_MS);
            const pos = Math.min(100, (offsetDays / rangeDays) * 100);
            const color = statusColor(dlMs, f.done);
            const label = statusLabel(dlMs, f.done);
            const isOverdue = !f.done && dlMs < today.getTime();
            return (
